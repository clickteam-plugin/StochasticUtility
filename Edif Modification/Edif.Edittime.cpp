...
HMENU WINAPI DLLExport GetConditionMenu(mv _far *mV, fpObjInfo oiPtr, LPEDATA edPtr)
{
	...
	if ( IS_COMPATIBLE(mV) )
    {
        ...
        menucpy(Menu, edPtr->Edif.ConditionMenu);
		return Menu;
    }
#endif // !defined(RUN_ONLY)
	return NULL;
...
HMENU WINAPI DLLExport GetActionMenu(mv _far *mV, fpObjInfo oiPtr, LPEDATA edPtr)
{
    ...
    {
        HMENU Menu = CreatePopupMenu();
        menucpy(Menu, edPtr->Edif.ActionMenu);
        return Menu;
    }
#endif // !defined(RUN_ONLY)
	return NULL;
...
HMENU WINAPI DLLExport GetExpressionMenu(mv _far *mV, fpObjInfo oiPtr, LPEDATA edPtr)
{
    ...
    {
        HMENU Menu = CreatePopupMenu();
        menucpy(Menu, edPtr->Edif.ExpressionMenu);
		return Menu;
    }
#endif // !defined(RUN_ONLY)
	return NULL;
...
void WINAPI PrepareFlexBuild(LPMV pMV, LPEDATA edPtr, LPCWSTR wTempFolder)
{
	...
	/* LOOKI MODIFIACTION START */
	string CExtLoadPath = string(TempFolder) + "Extensions\\CExtLoad.as";
	FILE* CExtLoad = fopen(CExtLoadPath.c_str(),"rb");
	if(CExtLoad)
	{
		fseek(CExtLoad,0,SEEK_END);
		int CExtLoadSize = ftell(CExtLoad);
		rewind(CExtLoad);

		char* Buffer = new char[CExtLoadSize];
		fread(Buffer,CExtLoadSize,1,CExtLoad);

		const char Stochastic[37] = "object=new CRunStochastic Utility();";
		
		char* StrPos = strstr(Buffer,Stochastic);
		//We found the string!
		if(StrPos)
		{
			memcpy(StrPos,"object=new CRunStochasticUtility ();",36);
		}

		//Write modified file
		CExtLoad = freopen(CExtLoadPath.c_str(),"wb",CExtLoad);
		fwrite(Buffer,CExtLoadSize,1,CExtLoad);

		delete[] Buffer;
		fclose(CExtLoad);
	}
	/* LOOKI MODIFICATION END */
